...
<x-filament-panels::page>
    ...
    <section>
        <h2>Blade</h2>
        <div data-empty-state-row="blade">
            @foreach ($this->getEmptyStateCases() as $attributes)
                @php
    ...
    @foreach (['react', 'vue', 'svelte'] as $framework)
        <section
            x-data="{
                ...
                isDestroyed: false,
                async init() {
                    const { default: mount } = await import(@js(FilamentAsset::getScriptSrc('empty-state', 'tests/empty-state')))
                    if (this.isDestroyed) return
                    this.renderer = mount(this.$refs.host, @js($framework), @js($this->getEmptyStateCases()))
                },
                destroy() {
        ...
        >
            ...
            <div
                wire:ignore
                x-ref="host"
                data-empty-state-row="{{ $framework }}"
            ></div>
