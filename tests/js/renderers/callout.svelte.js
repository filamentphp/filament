...
import ReactCallout from '../../../packages/support/resources/js/react/Callout'
import VueCallout from '../../../packages/support/resources/js/vue/Callout.vue'
import SvelteCallout from './callout.svelte'

export default function mountCallouts(host, framework, cases) {
    const renderers = cases.map((attributes) => {
        ...
        const component = mount(SvelteCallout, { target: container, props })
        return {
            update: (attributes) => {
                const next = {
                    ...prepare(attributes),
    ...
    return {
        ...
        reset: () => {
            host.firstElementChild.dataset.actions = '0'
            renderers[0].update(cases[0])
        },
