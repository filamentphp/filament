...
const renderer: JsFieldRenderer<Value, Configuration, Methods> = ({
    host,
    props,
    utilities,
}) => {
    ...
    // @ts-expect-error Renderer boolean props use predicate names, not HTML attribute names.
    props.disabled
    props.onChange({ title: 'Valid', tags: [] })
    props.onChange({ ...props.value, title: 'Updated' })
    utilities.$set('tags', props.value.tags)
    props.configuration.price.toFixed(2)
