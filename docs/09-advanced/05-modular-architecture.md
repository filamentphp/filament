...
## Registering render hooks from modules

Modules can add content to [render hooks](render-hooks) specific to the panels they're registered on:
...
public function boot(Panel $panel): void
{
    FilamentView::registerRenderHook(
        ...
        fn (): string => Blade::render('<x-alerts::notification-banner />'),
    );
}
```

## Registering assets from modules

When your module includes custom CSS or JavaScript, register them in your service provider using [Filament's asset system](assets):
...
class AlertsServiceProvider extends PackageServiceProvider
{
    ...
    public function packageBooted(): void
    {
        FilamentAsset::register(
            assets: [
                Css::make('alerts', __DIR__ . '/../../resources/dist/alerts.css'),
                Js::make('alerts', __DIR__ . '/../../resources/dist/alerts.js'),
            ],
            package: 'my-app/alerts',
        );
    }
}
```
