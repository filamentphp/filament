...
In Vue, use `defaultChecked` without `v-model` for an independent native reset default. Model updates also update the native checked default, so combining `v-model` and `defaultChecked` does not preserve a separate initial value.
