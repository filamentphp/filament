...
By default the chart type is set to `line`. So based on the chart class you extend set the chart `$type` property as well.
Suppose we wanted to extend the `Filament\Widgets\DoughnutChartWidget` then we will set the type to `doughnut`:

```php
use Filament\Widgets\DoughnutChartWidget;

class SessionsByDeviceChart extends DoughnutChartWidget
{
    protected static ?string $type = 'doughnut'; // [tl! **]
    ...
}
```
