...
<li x-data="{ label: {{ json_encode($label) }} }" class="filament-sidebar-group">
    @if ($label)
        ...
        >
            ...
            <x-heroicon-o-chevron-down class="w-3 h-3 text-gray-600 dark:text-gray-300" x-show="$store.sidebar.groupIsCollapsed(label)" />
            <x-heroicon-o-chevron-up class="w-3 h-3 text-gray-600 dark:text-gray-300" x-show="! $store.sidebar.groupIsCollapsed(label)" />
        </button>
    @endif
