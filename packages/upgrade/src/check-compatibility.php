...
render('<p class="text-blue font-bold">Checking Laravel version compatibility with v6...</p>');
...
if ($laravelVersion !== null && version_compare($laravelVersion, '11.28.0', '<')) {
    ...
    render("<p>Detected Laravel {$laravelVersion}. Filament v6 targets Laravel v11.28+ to rely on framework changes and fixes introduced in v11.28 and later. Please upgrade Laravel to at least v11.28 before continuing.</p>v");
    render(<<<'HTML'
        <p class="bg-red-600 text-red-50 mt-1">
...
render('<p class="text-blue font-bold">Checking plugin compatibility with v6...</p>');
...
// Initialize a shared global cache for Packagist data to be reused by the bin/filament-v6 script.
        ...
        if ($compatibility === null && $versions) {
            foreach ($versions as $checkingVersion) {
                ...
                foreach ($requires as $dep => $constraint) {
                    ...
                    if (preg_match("/\^\s*v?6(?:\.|$)|~\s*v?6(?:\.|$)|>=\s*v?6(?:\.|$)/", (string) $constraint)) {
                        $compatibility = [
...
if ($incompatiblePlugins) {
    ...
    render('<p>The following plugins are incompatible with Filament v6 and need to be removed before upgrading:</p>');
    render("<p class=\"text-red\">{$pluginList}</p>");
    render('<p>You could temporarily remove them from your composer.json file until they\'ve been upgraded, replace them with a similar plugin that is compatible with v6, wait for the plugins to be upgraded before upgrading your app, or even write PRs to help the authors upgrade them.</p>');
