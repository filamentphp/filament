...
- The `register()` method allows you to use any [configuration](configuration) option that is available to the panel. This includes registering [resources](resources), [custom pages](pages), [themes](themes), [render hooks](configuration#render-hooks) and more.
