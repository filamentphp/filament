...
The primary dashboard shown to a user is the first one they have access to (controlled by `canAccess()` method, that is available by all classes in the `Pages` hierarchy), according to the defined sort order.
