@php
    ...
    $items = $userMenu['onlyUserMenuItems']
        ? $items
        : \Illuminate\Support\Arr::except($items, ['account', 'logout', 'profile']);
@endphp
            ...
            <x-filament::dropdown.list>
                ...
                @foreach ($items as $key => $item)
                    ...
                    <x-filament::dropdown.list.item :action="$itemPostAction" :color="$item->getColor()" :href="$item->getUrl()"
                        :icon="$item->getIcon()" :method="filled($itemPostAction) ? 'post' : null" :tag="filled($itemPostAction) ? 'form' : 'a'" :target="$item->shouldOpenUrlInNewTab() ? '_blank' : null">
                        {{ $item->getLabel() }}
                    </x-filament::dropdown.list.item>
                @endforeach
                ...
                <x-filament::dropdown.list>
                    @foreach ($items as $key => $item)
                        @php
                            $itemPostAction = $item->getPostAction();
                        @endphp
                        @if ($hasProfileItem && $profileItem === $item)
                            <x-filament::dropdown.list>
                                <x-filament::dropdown.list.item :color="$profileItem?->getColor()" :icon="$profileItem?->getIcon() ??
                                    (\Filament\Support\Facades\FilamentIcon::resolve(
                                        'panels::user-menu.profile-item',
                                    ) ??
                                        'heroicon-m-user-circle')" :href="$profileItemUrl ?? filament()->getProfileUrl()"
                                    :target="$profileItem?->shouldOpenUrlInNewTab() ?? false ? '_blank' : null" tag="a">
                                    {{ $profileItem?->getLabel() ?? (($profilePage ? $profilePage::getLabel() : null) ?? filament()->getUserName($user)) }}
                        ...
                        @elseif ($logoutItem && $logoutItem === $item)
                            <x-filament::dropdown.list.item :action="$logoutItem?->getUrl() ?? filament()->getLogoutUrl()" :color="$logoutItem?->getColor()" :icon="$logoutItem?->getIcon() ??
                                (\Filament\Support\Facades\FilamentIcon::resolve('panels::user-menu.logout-button') ??
                                    'heroicon-m-arrow-left-on-rectangle')"
