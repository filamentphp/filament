...
<li @class([
    ...
    <a {{ \Filament\Support\generate_href_html($url, $shouldOpenUrlInNewTab) }}
        ...
        @if (filament()->isSidebarCollapsibleOnDesktop()) x-data="{ tooltip: false }"
            x-effect="
            ...
            @if (filament()->hasCollapsibleNavigationWithText())
                :class="{ 'flex-col' : !$store.sidebar.isOpen }" @endif
        @endif
        @class([
        ...
        @if (filled($icon))
            <x-filament::icon :icon="$active && $activeIcon ? $activeIcon : $icon" @class([
        ...
        <span
            ...
            @class([
                ...
                'text-xs text-center text-ellipsis w-16' =>
