...
> If you see anything missing from this guide, please do not hesitate to [make a pull request](https://github.com/filamentphp/filament/edit/3.x/packages/app/docs/14-upgrade-guide.md) to our repository! Any help is appreciated!
...
Before you can create the new context provider, make sure that you've got Filament v3 installed with Composer. Then, run the following command:

```bash
php artisan filament:install --app`
```

A new `app/Providers/Filament/AdminContextProvider.php` file will be created, ready for you to transfer over your old configuration from the `config/filament.php` file.
...
- `canAccessFilament()` has a new `\Filament\Context $context` parameter
...
- `getRouteBaseName()` has a new `?string $context` parameter
...
- `getRouteName()` has a new `?string $context` parameter
