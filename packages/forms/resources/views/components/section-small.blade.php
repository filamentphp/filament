<div @if ($isCollapsible()) x-data="{ isCollapsed: {{ $isCollapsed() ? 'true' : 'false' }} }"
        x-on:open-form-section.window="if ($event.detail.id == $el.id) isCollapsed = false"
        x-on:collapse-form-section.window="if ($event.detail.id == $el.id) isCollapsed = true"
        x-on:toggle-form-section.window="if ($event.detail.id == $el.id) isCollapsed = ! isCollapsed"
        x-on:expand-concealing-component.window="
            if ($event.detail.id === $el.id) {
                isCollapsed = false

                setTimeout(() => $el.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'start' }), 100)
            }
        " @endif
    id="{{ $getId() }}"
    {{ $attributes->merge($getExtraAttributes())->class([
            'rounded-xl border border-gray-300 filament-forms-section-component',
            'dark:border-gray-600' => config('forms.dark_mode'),
        ]) }}
    {{ $getExtraAlpineAttributeBag() }}>
    <div @class([
        'flex items-center px-2 rtl:space-x-reverse overflow-hidden rounded-t-xl min-h-[40px] filament-forms-section-header-wrapper',
    ])
        @if ($isCollapsible()) x-bind:class="{ 'rounded-b-xl': isCollapsed }"
        x-on:click="
                isCollapsed = ! isCollapsed

                if (isCollapsed) {
                    return
                }

                setTimeout(
                    () => $el.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'start' }),
                    100,
                )
            " @endif>
        <div @class([
            'flex-1 filament-forms-section-header',
            'cursor-pointer' => $isCollapsible(),
        ])>
            <h3 class="text-m font-bold tracking-tight pointer-events-none">
                {{ $getHeading() }}
            </h3>

            @if ($description = $getDescription())
                <p class="text-gray-500 pointer-events-none">
                    {{ $description }}
                </p>
            @endif
        </div>

        @if ($isCollapsible())
            <button x-on:click.stop="isCollapsed = ! isCollapsed"
                x-bind:class="{
                    '-rotate-180': !isCollapsed,
                }" type="button"
                @class([
                    'flex items-center justify-center w-5 h-5 transform rounded-full text-primary-500 hover:bg-gray-500/5 focus:bg-primary-500/10 focus:outline-none',
                    '-rotate-180' => !$isCollapsed(),
                ])>
                <svg class="h-7 w-7" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </button>
        @endif
    </div>

    <div @if ($isCollapsible()) x-bind:class="{ 'invisible h-0 !m-0 overflow-y-hidden': isCollapsed }"
            x-bind:aria-expanded="(! isCollapsed).toString()"
            @if ($isCollapsed()) x-cloak @endif
        @endif
        class="filament-forms-section-content-wrapper"
        >
        <div class="px-2 pb-4 filament-forms-section-content">
            {{ $getChildComponentContainer() }}
        </div>
    </div>
</div>
