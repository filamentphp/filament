    ...
    >
        ...
        <div class="flex-1">
            <input
                ...
                <Bny8qvcWKJAwnPHkpyZftRUwDzKMPACHuBxaJ9IN7cCs8t2JI4LwsHD41yDr310wfl52ZoNqcdNqez05tOuErzhUXGm5vnj3B
            />
            ...
            {{
                $getExtraInputAttributeBag()->class([
                    ...
                    'dark:bg-gray-700 dark:text-white' => config(
                        'forms.dark_mode',
                    ),
                ])
            }}
            x-bind:class="{ 'border-gray-300 focus:border-primary-500
            focus:ring-primary-500': ! (
            @js($getStatePath())
            in $wire.__instance.serverMemo.errors), 'dark:border-gray-600
            dark:focus:border-primary-500': ! (
            @js($getStatePath())
            in $wire.__instance.serverMemo.errors) &&
            @js(config('forms.dark_mode'))
            , 'border-danger-600 ring-danger-600 focus:border-danger-500
            focus:ring-danger-500': (
            @js($getStatePath())
            in $wire.__instance.serverMemo.errors), 'dark:border-danger-400
            dark:ring-danger-400 dark:focus:border-danger-500
            dark:focus:ring-danger-500': (
            @js($getStatePath())
            in $wire.__instance.serverMemo.errors) &&
            @js(config('forms.dark_mode'))
            , }" />
