...
function toggleCodeBlock (editor) {
  ...
  if (is_code === 'single') {
    ...
    const next_line_last_tok =
                next_line &&
                cm.getTokenAt({
                  line: block_end + 1,
                  ch: next_line.text.length - 1
                })
    const next_line_indented =
                next_line_last_tok &&
        ...
        if (!parentEl.hasAttribute('data-img-src')) {
          ...
          if (srcAttr && srcAttr.length >= 2) {
            ...
            if (options.imagesPreviewHandler) {
              const newSrc = options.imagesPreviewHandler(
                srcAttr[1]
              )
...
EasyMDE.prototype.createStatusbar = function (status) {
  ...
  const items = []
  let i, onUpdate, onActivity, defaultValue
