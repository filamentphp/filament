...
class FileUpload extends BaseFileUpload
{
    ...
    public function imageEditorMode(int $mode): static
    {
        if (! in_array($mode, [1, 2, 3])) {
            throw new InvalidArgumentException("The file upload editor mode must be either 1, 2 or 3. [{$mode}] given, which is unsupported. Mode 0 is not supported, as it does not allow configuration via manual inputs.");
        }
