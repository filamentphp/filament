...
You can use `../` to go up a level in the data structure, so `$get('../client_id')` (in repeater) is `$get('repeater.client_id')` (outside repeater) and `$get('../../client_id')` (in repeater) is `$get('client_id')` (outside repeater).
