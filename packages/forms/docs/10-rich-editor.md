...
RichContentRenderer::make($record->content)
    ->mergeTags([
        ...
        'formatted_greeting' => new HtmlString('<strong>Welcome back!</strong>'), // HTML content
...
You can also create custom classes that implement the `Htmlable` interface:

```php
use Illuminate\Contracts\Support\Htmlable;

class UserBadge implements Htmlable
{
...
RichContentRenderer::make($record->content)
    ->mergeTags([
        'user_badge' => new UserBadge($record->user),
    ])
    ->toHtml()
```
