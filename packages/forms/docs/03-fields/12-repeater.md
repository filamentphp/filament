...
Fields which provide selection of predefined options like Select, Radio, etc. provide a `distinctSelection()` method, which enforces only allowing each option to be selected in one instance of a Repeater.
...
Additionally, there is a `distinct()` [validation](../05-validation.md#distinct), which is automatically applied when using `distinctSelection()` but may be applied independently.
