...
The command creates a PHP field class and a text-input renderer in `resources/js/filament/forms/components/`, without a Blade view. It installs missing development dependencies, updates recognizable Vite configurations, and offers to compile your assets. Existing compatible dependency declarations keep their version constraints and their `package.json` sections. Incompatible dependencies require a manual upgrade instead of being replaced automatically. Use the generated class in your schema as shown above, then replace the starter input with your own UI.
...
- Svelte: `location-picker.svelte.js` and `LocationPicker.svelte`. With `--typescript`, the entry uses `.svelte.ts` and the component uses `<script lang="ts">`.
...
const props = defineProps(['value', 'id', 'isDisabled', 'isReadOnly', 'isRequired', 'isInvalid', 'ariaDescribedBy', 'onChange', 'onBlur'])
...
Create `resources/js/fields/LocationPicker.svelte`:

```svelte
<script>
    let { value, id, isDisabled, isReadOnly, isRequired, isInvalid, ariaDescribedBy, onChange, onBlur } = $props()
...
export default function mountLocationPicker({ host, props: initialProps }) {
    ...
    const update = (props) => {
        ...
        coordinates.forEach((coordinate) => {
            ...
            input.disabled = currentProps.isDisabled
            input.readOnly = currentProps.isReadOnly
            input.required = currentProps.isRequired
            input.setAttribute('aria-invalid', String(currentProps.isInvalid))
...
Apply `id`, `isDisabled`, `isReadOnly`, `isRequired`, `isInvalid`, and `ariaDescribedBy` to your inputs as shown in the examples. Map the boolean props to the input's `disabled`, `readonly`, `required`, and `aria-invalid` attributes. Use `id` on the primary input to associate Filament's label, and give additional inputs their own labels. `ariaDescribedBy` associates the wrapper's helper and validation text; append any additional description IDs instead of replacing it. If you override the field wrapper, preserve its supplied `descriptionId` around that text.

### Passing configuration from PHP

You can pass configuration to your renderer using `rendererConfiguration()`:
...
JsField::make('location')
    ->renderer(Vite::asset('resources/js/fields/location-picker.jsx'))
    ->rendererConfiguration(['zoom' => 12])
```

Read it from `props.configuration.zoom` in your component. Configuration is separate from built-in props such as `value` and `onChange`, so it cannot override the state binding. Only pass JSON-serializable data intended for the browser, not secrets. Modifying configuration in JavaScript does not write back to PHP.

In the Vue example, add `configuration` to `defineProps()`. In the Svelte example, read `configuration` from `$props()` alongside `value` and the other props.

<UtilityInjection set="formFields" version="4.x">As well as allowing a static value, the `rendererConfiguration()` method also accepts a function to dynamically calculate it. You can inject various utilities into the function as parameters.</UtilityInjection>
...
When the field renders on the server, Filament reevaluates the closure and passes changed configuration to `update(props)`. This preserves the mounted component and its local state. A deferred country edit reaches `props.configuration.country` on the next server render; use [`live()`](overview#the-basics-of-reactivity) on the country field if you need it sooner. A request that skips rendering this field does not update its PHP-derived configuration.

Treat configuration as immutable. Pass `null` to `rendererConfiguration()` to clear it. For a [reusable field](#building-reusable-plugin-fields), override `getRendererConfiguration(): array` instead; its default is `[]`.
...
Typed starters install `typescript@^6.0` because Vue and Svelte tooling still rely on its compiler API. Use the same version constraint when installing dependencies manually, including for React or framework-free fields in an application that also uses Vue or Svelte.

New `tsconfig.json` files use `"jsx": "react-jsx"` so Vite can compile React TSX in production. Existing JSX settings are preserved. If your configuration uses `"jsx": "preserve"`, configure a JSX transformer such as `@vitejs/plugin-react`, or change that setting to `"react-jsx"`. See the [Vite module setup](../advanced/assets#building-lazy-loaded-es-modules) for the remaining build configuration.

Vite transpiles TypeScript but does not type-check it. Run `npx tsc --noEmit` for React and framework-free renderers. For Vue, install `vue-tsc` as a development dependency and run `npx vue-tsc --noEmit`; for Svelte, install `svelte-check` and run `npx svelte-check --tsconfig ./tsconfig.json`. Use the framework-specific checker to check `.vue` or `.svelte` components, not just their entry files.

If your application uses multiple frameworks, scope each checker to files it understands. For example, `svelte-check` cannot resolve Vue components imported by other renderers. Create `tsconfig.svelte.json` and run `npx svelte-check --tsconfig ./tsconfig.svelte.json`:

```json
{
    "extends": "./tsconfig.json",
    "include": ["resources/js/**/*.svelte", "resources/js/**/*.svelte.ts"]
}
```
...
type Configuration = { zoom: number }

const mountLocationPicker: JsFieldRenderer<Location, Configuration> = ({ host, props, utilities }) => {
    // Mount your component with `props` and `utilities`.

    return {
        update(nextProps) {
            // Update it with `nextProps.value` and `nextProps.configuration`.
        },
        destroy() {
...
For [exposed PHP methods](#calling-an-exposed-method-from-a-framework), supply a third type argument, such as `JsFieldRenderer<Location, Configuration, Methods>`. Declare the methods your field exposes; PHP signatures are not automatically translated into TypeScript:
...
Clean up subscriptions, event listeners, and framework roots in `destroy()`, including when the host is already detached. Stop DOM work synchronously even if cleanup returns a promise. Changes to the renderer URL or state binding configuration remount the renderer with current state; changes to `rendererConfiguration()` and input settings such as `isInvalid`, `isRequired`, `isDisabled`, and `isReadOnly` update it in place. Keep persistent data in the field value, not only in component-local state.

#### Understanding the boundary with Blade and Alpine

Render only inside `host`, whose contents are ignored by Livewire. Do not change its parent or siblings. PHP records, operations, closures, Blade slots, and rendered Filament child schemas are not automatically available inside it. Compute serializable data in `rendererConfiguration()` and keep server-rendered actions, hints, and child schemas outside the host. Filament and Livewire still own field state, validation, authorization, and HTTP responses.
