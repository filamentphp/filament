...
Using `->multiple()` on the select component allows you to select multiple values from a list of predefined options:
...
Along with `->multiple()`, you may employ the `relationship()` method of the `Select` to configure a relationship to automatically retrieve and save options from:
