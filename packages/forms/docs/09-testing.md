...
If you need to make assertions on a layout component rather than a field, you may use `assertFormComponentExists()`.  As layout components do not have field names, this method uses either the key (if present) or ID of the component.
...
As the key/ID of a layout component is typically generated by Filament (for instance by slugifying the title), you may need to specify a key in your schema for testing purposes.

As with `assertFormFieldExists()`, you may also provide a callback for making additional truth tests.

```php
\Filament\Forms\Components\Section::make('Foo Bar')
