...
class TextColumn extends Column implements HasEmbeddedView
{
    ...
    public function toEmbeddedHtml(): string
    {
        ...
        $formatState = function (mixed $stateItem, ?Model $relationshipRecord = null, mixed $formattedState = null) use ($shouldOpenUrlInNewTab): string {
            ...
            $item .= e($formattedState ?? $this->formatState($stateItem, $relationshipRecord));
        ...
        if (($stateCount > 1) && (! $isListWithLineBreaks) && (! $isBadge)) {
            ...
            $stateCount = 1;
            $relationshipRecords = [];
            $formatState = fn (mixed $stateItem, ?Model $relationshipRecord = null, mixed $formattedState = null): string => $stateItem;
        ...
        $getStateItem = function (mixed $stateItem, ?Model $relationshipRecord = null, mixed $formattedState = null) use ($iconPosition, $isBadge, $lineClamp): array {
            return $this->withEvaluatingRelationshipRecord($relationshipRecord, function () use ($stateItem, $relationshipRecord, $formattedState, $iconPosition, $isBadge, $lineClamp): array {
                ...
                $iconHtml = generate_icon_html($this->getIcon($stateItem, $relationshipRecord), attributes: (new FilamentComponentAttributeBag)
                ...
                if ($isCopyable) {
                    $copyableStateJs = Js::from($this->getCopyableState($stateItem) ?? $formattedState ?? $this->formatState($stateItem, $relationshipRecord));
        ...
        if (
            ...
            $stateItemFormattedState = $isCollapsedList ? null : $this->formatState($stateItem, $relationshipRecord);
            [
            ...
            ] = $getStateItem($stateItem, $relationshipRecord, $stateItemFormattedState);
            ...
            <div <?= $attributes
                ...
                <?= $formatState($stateItem, $relationshipRecord, $stateItemFormattedState) ?>
