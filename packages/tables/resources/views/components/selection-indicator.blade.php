...
<tr x-cloak {{ $attributes->class(['bg-primary-500/10 filament-tables-selection-indicator']) }}>
    <td class="px-4 py-2 whitespace-nowrap text-sm" colspan="{{ $colspan }}">
        <div>
            ...
            <span @class(['dark:text-white' => config('tables.dark_mode')]) x-text="$pluralize('{{__('tables::table.selection_indicator.selected_count')}}', selectedRecords.length, {'count': selectedRecords.length})"></span>
