...
@props([
    ...
    'applyActionPosition' => ColumnManagerApplyActionPosition::Footer,
])

<div class="fi-ta-col-manager">
    <div x-data="filamentTableColumnManager({
        ...
        <div class="fi-ta-col-manager-header">
            <{{ $headingTag }} class="fi-ta-col-manager-heading">
                ...
                @if (
                    $resetActionPosition === ColumnManagerResetActionPosition::Header ||
                        ($applyActionPosition === ColumnManagerApplyActionPosition::Header && $applyAction->isVisible()))
                    <div class="fi-ta-col-manager-actions-ctn">
                        @if ($applyActionPosition === ColumnManagerApplyActionPosition::Header && $applyAction->isVisible())
                            <x-filament::link :attributes="\Filament\Support\prepare_inherited_attributes(
                                new ComponentAttributeBag([
                                    'color' => 'primary',
                                    'tag' => 'button',
                                    'x-on:click' => 'applyTableColumnManager; close()',
                                ]),
                            )">
                                {{ __('filament-tables::table.column_manager.actions.apply.label') }}
                            </x-filament::link>
                        @endif
        ...
        <div @if ($hasReorderableColumns) x-sortable
        ...
        data-sortable-animation-duration="{{ $reorderAnimationDuration }}" @endif
            {{ new ComponentAttributeBag()->grid($columns, GridDirection::Column)->class(['fi-ta-col-manager-items']) }}>
            <template x-for="(column, index) in columns.filter((column) => ! column.isHidden && column.label)"
                x-bind:key="(column.type === 'group' ? 'group::' : 'column::') + column.name + '_' + index">
                <div
                    @if ($hasReorderableColumns) x-bind:x-sortable-item="column.type === 'group' ? 'group::' + column.name : 'column::' + column.name" @endif>
                        ...
                        <div class="fi-ta-col-manager-group">
                            <div class="fi-ta-col-manager-item">
                                ...
                                @if ($hasReorderableColumns)
                                    <button x-sortable-handle x-on:click.stop
                                        class="fi-ta-col-manager-reorder-handle fi-icon-btn" type="button">
                            ...
                            @if (!$this->getTable()->hasColumnManagerGroupsOnly())
                                <div @if ($hasReorderableColumns) x-sortable
                                ...
                                data-sortable-animation-duration="{{ $reorderAnimationDuration }}" @endif
                                    ...
                                    "
                                        ...
                                        <div
                                            @if ($hasReorderableColumns) x-bind:x-sortable-item="'column::' + groupColumn.name" @endif>
                                            <div class="fi-ta-col-manager-item">
                                                <label class="fi-ta-col-manager-label">
                                                    @if ($hasToggleableColumns)
                                                        <input type="checkbox" class="fi-checkbox-input fi-valid"
                                                            x-bind:id="'column-' + groupColumn.name.replace('.', '-')"
                                                            x-bind:checked="(getColumn(groupColumn.name, column.name) || {}).isToggled
                                                                ||
                                                                false"
                                                            x-bind:disabled="(getColumn(groupColumn.name, column.name) || {})
                                                            .isToggleable === false"
                                                            x-on:change="toggleColumn(groupColumn.name, column.name)" />
                                                    @endif

                                                    <span x-html="groupColumn.label"></span>
                                                </label>
        ...
        @if (
            $applyActionPosition === ColumnManagerApplyActionPosition::Footer ||
                $resetActionPosition === ColumnManagerResetActionPosition::Footer)
            <div class="fi-ta-col-manager-actions-ctn">
                @if ($applyActionPosition === ColumnManagerApplyActionPosition::Footer && $applyAction->isVisible())
                    {{ $applyAction }}
                @endif

                @if ($resetActionPosition === ColumnManagerResetActionPosition::Footer)
                    <x-filament::button color="danger" wire:click="resetTableColumnManager"
                        x-on:click="resetDeferredColumns">
                        {{ __('filament-tables::table.column_manager.actions.reset.label') }}
                    </x-filament::button>
                @endif
